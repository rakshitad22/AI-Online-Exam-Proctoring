...
class AbnormalActivityDetector:
    ...
    def load_model(self):
        """
        Loads OpenCV Haar Cascades and object detection models into memory lazily.
        """
        if self.is_loaded:
            return
        ...
        try:
            ...
            # Fallback loader if Haar path is missing in serverless container
    ...
    def process_frame(self, frame_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            ) if self.face_cascade and not self.face_cascade.empty() else []
            ...
            if len(nms_boxes) == 0 and self.profile_face_cascade and not self.profile_face_cascade.empty():
    ...
    def _detect_talking_signature(self, mouth_roi_gray: np.ndarray) -> bool:
        ...
        """
        if mouth_roi_gray is None or mouth_roi_gray.size == 0 or self.smile_cascade is None or self.smile_cascade.empty():
            return False
